...
export default async function AnuncisAdminPage({
  searchParams,
}: {
  searchParams: Promise<{ saved?: string; deleted?: string }>;
}) {
  const sp = await searchParams;
  const items = await readCollection("anuncis");

  return (
    <div>
      <PageHeader
        title="Anuncis"
        kicker="Punt de trobada"
        actions={
          <Link
            href="/admin/anuncis/nou"
      ...
      <CollectionList items={items} entity="anuncis" />
    </div>
  );
}
